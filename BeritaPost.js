      ...
      <div className={classes.beritaContentWrapper}>
        <Grid container spacing={5}>
          ...
          <Grid item md={3}>
            ...
            {otherArticles.map((beritaPost) => (
              <Box key={beritaPost.id} mb={3}>
                <BeritaCard
                  title={beritaPost.title}
                  snippet={beritaPost.snippet}
                  date={beritaPost.date}
                  url={`${beritaPost.url}${beritaPost.params}`}
                />
              </Box>
            ))}
          </Grid>
...
export default withStyles(styles, { withTheme: true })(BeritaPost);
